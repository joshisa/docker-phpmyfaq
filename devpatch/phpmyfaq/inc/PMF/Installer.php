...
class PMF_Installer
{
    ...
    public function startInstall(Array $DB = null)
    {
        ...
        $dbSetup['dbPort'] = PMF_Filter::filterInput(INPUT_POST, 'sql_port', FILTER_SANITIZE_STRING);
        if (is_null($dbSetup['dbPort']) && !PMF_System::isSqlite($dbSetup['dbType'])) {
            echo "<p class=\"alert alert-danger\"><strong>Error:</strong> Please add a database port.</p>\n";
            PMF_System::renderFooter(true);
        }
        ...
        try {
            $db->connect($dbSetup['dbServer'], $dbSetup['dbPort'], $dbSetup['dbUser'], $dbSetup['dbPassword'], $dbSetup['dbDatabaseName']);
        ...
        $db = PMF_Db::factory($dbSetup['dbType']);
        $db->connect($DB['server'], $DB['port'], $DB['user'], $DB['password'], $DB['db']);
        if (!$db) {
            printf("<p class=\"alert alert-danger\"><strong>DB Error:</strong> %s</p>\n", $db->error());
            $this->_system->cleanInstallation();
            PMF_System::renderFooter(true);
